...
|---|---|---|---|
| OpenAI LLM | Natural language post parsing, icebreakers, moderation | `OPENAI_API_KEY` | `LLMLog` |
...
- Runtime behavior: OpenAI calls are logged when a key is available; fallback is used when no key is set or an API call fails.

Recommended report comparison:

| Dimension | OpenAI LLM | Rule baseline |
|---|---|---|
